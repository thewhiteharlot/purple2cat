...
@bot.on(admin_cmd(pattern=r"tlock (.*)"))
@bot.on(sudo_cmd(pattern=r"tlock (.*)", allow_sudo=True))
async def _(event):
        if event.fwd_from:
            return
        inputthings = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
        peer_id = event.chat_id
        if not event.is_group:
            return await edit_delete(event, "`Idiot! ,This is not a group to lock things `")
        chat_per = (await event.get_chat()).default_banned_rights
        ...
        changeinfo = chat_per.change_info
        ctime = None
        ctime = await extract_time(event, inputthings[0])
        if not ctime:
        ...
        try:
            ...
            await edit_or_reply(event, f"`Locked {locktype} for this chat for {inputthings[0]} time.`")
        except BaseException as e:
            await edit_delete(
                event,
                f"`Do I have proper rights for that ??`\n\n**Error:** `{str(e)}`",
